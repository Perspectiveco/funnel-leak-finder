...
If ads are connected: spend, CPM, CTR, CPL per campaign feeding this funnel, plus the actual ad creative for the top-spending ads (headline, primary text, thumbnail) so you can compare the promise in the ad against the first screen of the funnel. If the ad platform doesn't expose creative text, ask the user once for a screenshot of their top ad rather than guessing at what it promised.
